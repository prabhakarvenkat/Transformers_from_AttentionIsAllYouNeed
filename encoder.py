...
class EncoderBlock(nn.Module):
    def __init__(self):
        '''
        A Single Encoder block that can be stacked multiple times to form the encoder which uses two main blocks: 
        Self Attention  Feed Forward connected with some Residual Connections
...
class Encoder(nn.Module):
    ...
    def __init__(self, d_model: int, num_layers: int):
        ...
        '''
        super().__init__()
        self.layers = nn.ModuleList([EncoderBlock() for _ in range(num_layers)]) # List of encoder blocks
